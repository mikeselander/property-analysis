...
describe("Calculations Simple Unit Tests", () => {
	it("Calculates Monthly Expenses", () => {
		expect(calculateMonthlyFixedExpenses(1000, 5, 5, 5, 5, 50)).toEqual(250);
	});

	it("Calculates Monthly Cashflow", () => {
		expect(calculateMonthlyCashFlow(1500, 300, 2000)).toEqual(200);

		// @todo:: Also do negative
	});

	it("Calculates Monthly Insurance", () => {
		expect(calculateMonthlyInsurance(75, false, 200000, 20)).toEqual(75);

		expect(calculateMonthlyInsurance(75, true, 100000, 10)).toEqual(112.5);
	});

	it("Calculates Money Down", () => {
		expect(calculateMoneyDown(250000, 10, 5000, 5000)).toEqual(35000);
	});
});
...
describe("calculateMonthlyPandI", () => {
	it.each([
		...
		"calculates $%s price and %s% down and %s interest as %s",
		(purchasePrice, percentDown, interestRate, expected) => {
			expect(
				calculateMonthlyPandI(purchasePrice, percentDown, interestRate).toFixed(
					2
				)
			).toBe(expected);
		}
...
describe("calculatePrincipal", () => {
	it.each([
		...
		"calculates $%s price and %s% down as %s",
		(purchasePrice, percentDown, expected) => {
...
describe("calculateCapRate", () => {
	it.each([
		...
		"calculates $%d value and $%d monthly noi as %d",
		(moneyDown, monthlyCashFlow, expected) => {
...
describe("calculateMonthlyPmi", () => {
	it.each([
		...
		"calculates $%d price and %d% down as %d",
		(purchasePrice, percentDown, expected) => {
			expect(calculateMonthlyPmi(purchasePrice, percentDown).toFixed(2)).toBe(
				expected
			);
		}
	);
...
describe("calculateYearlyDepreciationWriteOff", () => {
	it.each([
		...
		if (basis) {
			expect(calculateYearlyDepreciationWriteOff(price, basis).toFixed(2)).toBe(
				expected
			);
		} else {
			expect(calculateYearlyDepreciationWriteOff(price).toFixed(2)).toBe(
				expected
			);
		}
	});
