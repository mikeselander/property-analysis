...
const OngoingCostsTab = ({ handleChange, values }: HouseTabProps) => {
	const calculateAmount = (value: string | number) =>
		formatter.format(convertPercentInteger(Number(value)) * values.monthlyRent);

	return (
		<Grid container justify="space-between" alignItems="flex-start" spacing={3}>
			<Grid item xs={5} sm={6}>
				<FormControl>
					<InputLabel htmlFor="adornment-vacancy">
						Vacancy Rate ({calculateAmount(values.vacancy)})
					</InputLabel>
					<Input
			...
			<Grid item xs={5} sm={6}>
				<FormControl>
					<InputLabel htmlFor="adornment-maintenance">
						Maintenance ({calculateAmount(values.maintenance)})
					</InputLabel>
					<Input
			...
			<Grid item xs={5} sm={6}>
				<FormControl>
					<InputLabel htmlFor="adornment-capEx">
						CapEx ({calculateAmount(values.capEx)})
					</InputLabel>
					<Input
						id="adornment-capEx"
						value={values.capEx}
			...
			<Grid item xs={5} sm={6}>
				<FormControl>
					<InputLabel htmlFor="adornment-management">
						Management ({calculateAmount(values.management)})
					</InputLabel>
					<Input
			...
			<Grid item xs={5} sm={6}>
				<FormControl>
					<InputLabel htmlFor="adornment-insuranceCost">
						Monthly Insurance
					</InputLabel>
					<Input
